...
class MoleculeDataset_graphclDouble(MoleculeDataset):
    ...
    def __init__(self,
        ...
        super(MoleculeDataset_graphclDouble, self).__init__(
    ...
    def get(self, idx):
        ...
        if self.aug_mode == 'no_aug':
            ...
            data1 = self.augmentations[n_aug1](data1)
            data2 = self.augmentations[n_aug2](data2)
        elif self.aug_mode == 'double':
            first_augmentations = [self.augmentations[0], self.augmentations[3], lambda x: x]
            second_augmentations = [self.augmentations[1], self.augmentations[2], lambda x: x]

            #n_aug = np.random.choice(3,1)[0]
            n_aug1 = np.random.choice(3,1)[0]
            n_aug2 = np.random.choice(3,1)[0]
            first_data1 = first_augmentations[n_aug1](data)
            first_data2 = first_augmentations[n_aug2](data)

            #n_aug = np.random.choice(3,1)[0]
            n_aug1 = np.random.choice(3,1)[0]
            n_aug2 = np.random.choice(3,1)[0]
            #second_data = second_augmentations[n_aug](data)
            second_data1 = second_augmentations[n_aug1](data)
            second_data2 = second_augmentations[n_aug2](data)  
        elif self.aug_mode == 'edgedeldouble':
            first_augmentations = [self.augmentations[0], self.augmentations[3], lambda x: x]
            second_augmentations = [self.augmentations[1], self.augmentations[2], lambda x: x]
            
            frag_data = self.edge_del(data)

            #n_aug = np.random.choice(3,1)[0]
            n_aug1 = np.random.choice(3,1)[0]
            n_aug2 = np.random.choice(3,1)[0]
            first_data1 = first_augmentations[n_aug1](data)
            first_data2 = first_augmentations[n_aug2](frag_data)
            ...
            #second_data = second_augmentations[n_aug](data)
            second_data1 = second_augmentations[n_aug1](data)
            second_data2 = second_augmentations[n_aug2](frag_data)  
        else:
            raise ValueError
        return data, first_data1, first_data2, data, second_data1, second_data2
