...
class Molecule3DDatasetFragRandomaug3d_4frag(MoleculeDataset):
    ...
    def __init__(self, root, n_mol,choose = 0,transform=None, seed=777,
        ...
        super(Molecule3DDatasetFragRandomaug3d_4frag, self).__init__(
            root, transform, pre_transform, pre_filter)

        if not empty:
            self.data, self.slices = torch.load(self.processed_paths[0])
            self.data0, self.slices0 = torch.load(self.processed_paths[0] + "_0")
    ...
    def get(self, idx):
        ...
        for key in self.data1.keys:
            item0, slices0 = self.data0[key], self.slices0[key]
            s = list(repeat(slice(None), item0.dim()))
            s[data0.__cat_dim__(key, item0)] = slice(slices0[idx], slices0[idx+1])
            data0[key] = item0[s]
        ...
        if self.aug_mode == 'no_aug':
            ...
            return aug_data, data, data0, data1, data2, data3
