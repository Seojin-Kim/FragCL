...
class Molecule3DDatasetFragRandomaug3d_brics(MoleculeDataset):
    ...
    def __init__(self, root, n_mol,choose = 0,transform=None, seed=777,
        ...
        super(Molecule3DDatasetFragRandomaug3d_brics, self).__init__(
            root, transform, pre_transform, pre_filter)
        self.brics = []
        self.brics_slice = []
        if not empty:
            self.data, self.slices = torch.load(self.processed_paths[0])
            for i in range(42068):
    ...
    def get(self, idx):
        data = Data()
        brics_datas = [Data(), Data(), Data(), Data(), Data(), Data(), Data(), Data(), Data(), Data()]
        #self.brics[idx]
        #self.brics_slice[idx]
        for key in self.data.keys:
            item, slices = self.data[key], self.slices[key]
            s = list(repeat(slice(None), item.dim()))
            s[data.__cat_dim__(key, item)] = slice(slices[idx], slices[idx+1])
            data[key] = item[s]
        for key in self.brics[idx].keys:
            #print(idx)
            #print(self.brics[idx])
            #print(self.brics_slice[idx])
            #print(self.brics_slice[idx]['x']-1)
            
            for i in range(min(len(self.brics_slice[idx]['x'])-1, 10)):
                item, slices = self.brics[idx][key], self.brics_slice[idx][key]
                s = list(repeat(slice(None), item.dim()))
                s[brics_datas[i].__cat_dim__(key, item)] = slice(slices[i], slices[i+1])
                brics_datas[i][key] = item[s]
            for i in range(len(self.brics_slice[idx]['x'])-1, len(brics_datas)):
                brics_datas[i] = Data(edge_attr=torch.empty((0,2), dtype=torch.long), x=torch.empty((0,2),dtype=torch.long), edge_index=torch.empty((2,0),dtype=torch.long), positions=torch.empty((0,3),dtype=torch.float))
        ...
        if self.aug_mode == 'no_aug':
            ...
            #aug_data1 = two_augmentations[n_aug1](data1.clone())
            #aug_data2 = two_augmentations[n_aug2](data2.clone())

            return aug_data, data, brics_datas[0], brics_datas[1], brics_datas[2], brics_datas[3], brics_datas[4], brics_datas[5], brics_datas[6], brics_datas[7], brics_datas[8], brics_datas[9]
