...
def train(loader, model, optimizer, device, old):
    ...
    for step, (batch, batch1, batch2) in enumerate(loader):
        ...
        if old == 1:
            ...
            #x2 = model.forward_cl(batch2.x, batch2.edge_index,
            #                      batch2.edge_attr, batch2.batch)
            x2, frag1, frag2 = model.weighted_forward_frag_cl(batch1.x, batch1.edge_index,
                                batch1.edge_attr, batch1.batch,
                                batch2.x, batch2.edge_index,
                                batch2.edge_attr, batch2.batch)
            
            loss = model.weighted_loss_cl_onlyneg(x, x2, frag1, frag2, batch1.batch, batch2.batch)
...
if __name__ == "__main__":
    ...
    if 'GEOM' in args.dataset:
        n_mol = args.n_mol
        root_2d = '{}/GEOM_2D_cut_singlebond_rdkit_0911'.format(args.data_folder, args.n_mol)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        pretrain_loss = train(loader, model, optimizer, device, args.old)
