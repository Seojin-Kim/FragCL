...
class graphcl_brics(nn.Module):

    def __init__(self, gnn):
        super(graphcl_brics, self).__init__()
        self.gnn = gnn
    ...
    def forward_frag_cl(self, frag0, frag1, frag2, frag3):
        ...
        x0 = self.gnn(frag0.x, frag0.edge_index, frag0.edge_attr)
        x0 = self.pool(x0, frag0.batch)

        x1 = self.gnn(frag1.x, frag1.edge_index, frag1.edge_attr)
        x1 = self.pool(x1, frag1.batch)

        x2 = self.gnn(frag2.x, frag2.edge_index, frag2.edge_attr)
        x2 = self.pool(x2, frag2.batch)

        x3 = self.gnn(frag3.x, frag3.edge_index, frag3.edge_attr)
        x3 = self.pool(x3, frag3.batch)
...
class graphcl3d_brics(nn.Module):
    ...
    def forward_frag_cl(self, frag0, frag1, frag2, frag3):
        ...
        x0 = self.gnn(frag0.x[:,0], frag0.positions, frag0.batch)
        x1 = self.gnn(frag1.x[:,0], frag1.positions, frag1.batch)
        x2 = self.gnn(frag2.x[:,0], frag2.positions, frag2.batch)
        x3 = self.gnn(frag3.x[:,0], frag3.positions, frag3.batch)
...
class graphcl(nn.Module):
    ...
    def forward_cl(self, x, edge_index, edge_attr, batch):
        r = self.gnn(x, edge_index, edge_attr)
        r = self.pool(r, batch)
        x = self.projection_head(r)
        return x#, r
    ...
    def loss_cl(self, x1, x2, frag1, frag2, frag3):
        T = 0.1
        ...
        frag2_matrix = torch.einsum('ik,jk->ij', x1, frag2) / \
                     torch.einsum('i,j->ij', x1_abs, frag2_abs)
        frag3_matrix = torch.einsum('ik,jk->ij', x1, frag3) / \
                     torch.einsum('i,j->ij', x1_abs, frag3_abs)

        sim_matrix = torch.exp(sim_matrix / T)
        
        frag1_matrix = torch.exp(frag1_matrix / T)
        frag2_matrix = torch.exp(frag2_matrix / T)
        frag3_matrix = torch.exp(frag3_matrix / T)
        neg1 = frag1_matrix[range(batch), range(batch)]
        neg2 = frag2_matrix[range(batch), range(batch)]
        neg3 = frag3_matrix[range(batch), range(batch)]
        pos_sim = sim_matrix[range(batch), range(batch)]
        loss = pos_sim / (sim_matrix.sum(dim=1) + neg1 + neg2 + neg3 - pos_sim)
    ...
    def weighted_forward_frag_cl_trisect(self, x1, edge_index1, edge_attr1, batch1, x2, edge_index2, edge_attr2, batch2, x3, edge_index3, edge_attr3, batch3):
        x1 = self.gnn(x1, edge_index1, edge_attr1)
        x1 = self.pool(x1, batch1)
        ...
        x3 = self.gnn(x3, edge_index3, edge_attr3)
        x3 = self.pool(x3, batch3)


        batch1_numatoms = torch.bincount(batch1)
        batch2_numatoms = torch.bincount(batch2)
        batch3_numatoms = torch.bincount(batch3)
        total_numatoms = batch1_numatoms + batch2_numatoms + batch3_numatoms
        batch1_ratio = batch1_numatoms / total_numatoms
        batch2_ratio = batch2_numatoms / total_numatoms
        batch3_ratio = batch3_numatoms / total_numatoms

        x = x1 * batch1_ratio.unsqueeze(1).repeat(1,x1.size(1)) + x2 * batch2_ratio.unsqueeze(1).repeat(1,x2.size(1)) + x3 * batch3_ratio.unsqueeze(1).repeat(1, x3.size(1))

        x = self.projection_head(x)
        frag1 = self.projection_head(x1)
        frag2 = self.projection_head(x2)
        frag3 = self.projection_head(x3)
        return x, frag1, frag2, frag3# x1, x2
    ...
    def weighted_loss_cl(self, x1, x2, frag1, frag2, frag3, batch1, batch2, batch3):
        T = 0.1
        batch, _ = x1.size()
        x1_abs = x1.norm(dim=1)
        x2_abs = x2.norm(dim=1)
